...
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
"""
def fiber_centers(r_aper, arr_size, mask_size):
    #Generating the dicts and setting up the bin mask
    #Bin mask will be the size of shape of the image

    if mask_size%10 > 0:
        return "Error: Bin Mask is not a multiple of 10!"
    else:
        x_cen = {}
        y_cen = {}

        img = np.zeros((mask_size, mask_size))
        x0 = mask_size/2
        y0 = mask_size/2
        ...
        if arr_size > 1:
            x_cen['2'], y_cen['2'] = (x0 - r_aper - np.round((r_aper * np.sin(np.pi/6)))) , (y0 + np.round(r_aper * np.cos(np.pi / 6)))
            x_cen['3'], y_cen['3'] =  x0 , (y0  + np.round(2 * (r_aper * np.cos(np.pi/6))))
            ...
            if arr_size > 7:
                ...
                x_cen['10'], y_cen['10'] = x_cen['2'], (2 * y_cen['3']) - y_cen['2'] + 1
        ...
        if arr_size == 1:
            x_cen['1'] = np.int(x_cen[str(1)])
            y_cen['1'] = np.int(y_cen[str(1)])
